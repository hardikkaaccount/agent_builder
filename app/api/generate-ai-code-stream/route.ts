...
`;
        ...
        if (isAnthropic) {
          ...
          actualModel = model.replace('bedrock/', '');
        } else {
          actualModel = model;
        }

        console.log(`[generate-ai-code-stream] Using provider: ${isAnthropic ? 'Anthropic' : isGoogle ? 'Google' : isOpenAI ? 'OpenAI' : isBedrock ? 'Bedrock' : 'Groq'}, model: ${actualModel}`);
...
It's better to have 3 complete files than 10 incomplete files.`
        ...
        // Add stopSequences only for models that support it (not for Bedrock models)
