...
function fixLucideReactImports(content: string): string {
  ...
  updatedContent = updatedContent.replace(lucideImportRegex, (match, imports) => {
    ...
    // If we have fixed imports, rebuild the import statement
    if (fixedImports.length > 0) {
      return `import { ${fixedImports.join(', ')} } from 'lucide-react'`;
    }
    
    // If no valid imports, remove the entire import line
    return '';
  });
  
  // Now replace all usages of remapped icons in the content
  for (const [oldIcon, newIcon] of Object.entries(iconReplacements)) {
    // Replace JSX component usage: <OldIcon /> or <OldIcon className="..." />
    const usageRegex = new RegExp(`<${oldIcon}\\b(\\s|/|>)`, 'g');
    updatedContent = updatedContent.replace(usageRegex, `<${newIcon}$1`);
  }
...
function parseAIResponse(response: string): ParsedResponse {
  ...
  for (const [path, { content, isComplete }] of fileMap.entries()) {
    ...
    const fixedContent = fixLucideReactImports(content);
    ...
    const filePackages = extractPackagesFromCode(fixedContent);
    ...
    for (const fileName of filesList) {
      ...
      if (fileContentMatch) {
        ...
        if (codeMatch) {
          ...
          const fixedFileContent = fixLucideReactImports(codeMatch[1].trim());
          sections.files.push({
            path: filePath,
          ...
          const filePackages = extractPackagesFromCode(fixedFileContent);
          for (const pkg of filePackages) {
            if (!sections.packages.includes(pkg)) {
              sections.packages.push(pkg);
              console.log(`[apply-ai-code-stream] Package detected from imports: ${pkg}`);
  ...
  while ((match = codeBlockRegex.exec(response)) !== null) {
    ...
    if (fileNameMatch) {
      ...
      if (!sections.files.some(f => f.path === filePath)) {
        const fixedCodeBlockContent = fixLucideReactImports(content);
        sections.files.push({
          path: filePath,
          content: fixedCodeBlockContent
        });

        // Extract packages
        const filePackages = extractPackagesFromCode(fixedCodeBlockContent);
        for (const pkg of filePackages) {
          if (!sections.packages.includes(pkg)) {
            sections.packages.push(pkg);
          }
