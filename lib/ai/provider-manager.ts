...
type ProviderName = 'openai' | 'anthropic' | 'groq' | 'google' | 'bedrock';
...
function getEnvDefaults(provider: ProviderName): { apiKey?: string; baseURL?: string } {
  ...
  switch (provider) {
    ...
    case 'bedrock':
      return {}; // Bedrock uses AWS credentials, not a simple API key
    default:
      return {};
  }
}
...
function getOrCreateClient(provider: ProviderName, apiKey?: string, baseURL?: string): ProviderClient {
  ...
  switch (provider) {
    ...
    case 'google':
      ...
      break;
    case 'bedrock':
      // Bedrock automatically uses AWS_BEARER_TOKEN_BEDROCK from environment
      client = createAmazonBedrock() as any;
      break;
    default:
      client = createGroq({ apiKey: effective.apiKey || getEnvDefaults('groq').apiKey, baseURL: effective.baseURL ?? getEnvDefaults('groq').baseURL });
  }
...
export function getProviderForModel(modelId: string): ProviderResolution {
  ...
  const isBedrock = modelId.startsWith('bedrock/');
  ...
  if (isBedrock) {
    const client = getOrCreateClient('bedrock');
    return { client, actualModel: modelId.replace('bedrock/', '') };
  }
